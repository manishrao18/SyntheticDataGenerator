...
class PIIFactory(ABC):
    def __init__(self, locale):
        ...
        # rules: list of callables that take (record: dict) and may modify it in-place
        self._rules = []
    ...
    def register_rule(self, rule_callable):
        """Register a custom rule. The callable receives the record dict and may modify it."""
        if callable(rule_callable):
            self._rules.append(rule_callable)

    def _register_default_rules(self):
        # add built-in rules
        self._rules.append(self._rule_marriage_count_consistency)
        self._rules.append(self._rule_family_consistency)
        self._rules.append(self._rule_income_worktype_consistency)
        self._rules.append(self._rule_vehicle_family_consistency)
        self._rules.append(self._rule_credit_networth_alignment)

    def enforce_rules(self, record):
        """Apply all registered rules to the record in order."""
        for rule in list(self._rules):
            try:
                rule(record)
            except Exception:
                # be resilient: log and continue
                logger.exception(f"Rule {getattr(rule, '__name__', str(rule))} failed for record: {record.get('Full Name')}")

    # --- Default rule implementations ---
    def _rule_marriage_count_consistency(self, record):
        ms = record.get('Marital Status')
        if ms not in ['Married', 'Divorced', 'Widowed']:
            record['Marriage Count'] = None

    def _rule_family_consistency(self, record):
        kids = int(record.get('Number of Kids') or 0)
        married = 1 if record.get('Marital Status') == 'Married' else 0
        base = 1 + married + kids
        fam = int(record.get('Family Members Count') or base)
        if fam < base:
            record['Family Members Count'] = base

    def _rule_income_worktype_consistency(self, record):
        work = record.get('Work Type')
        income = int(record.get('Annual Income (USD)') or 0)
        # reasonable minimums
        if work == 'Salaried' and income < 8000:
            # bump to minimum for salaried
            record['Annual Income (USD)'] = max(8000, income + random.randint(1000, 10000))
        if work == 'Business' and income < 10000:
            record['Annual Income (USD)'] = max(10000, income + random.randint(2000, 30000))
        if work == 'Unemployed':
            # ensure unemployed have low income and not main earner
            record['Annual Income (USD)'] = min(income, 5000)
            record['Is Main Earner'] = False
            record['Is Co-Earner'] = False

    def _rule_vehicle_family_consistency(self, record):
        fam = int(record.get('Family Members Count') or 1)
        vehicles = int(record.get('Personal Vehicle Count') or 0)
        if fam >= 4 and vehicles == 0:
            record['Personal Vehicle Count'] = 1

    def _rule_credit_networth_alignment(self, record):
        try:
            net = int(record.get('Net Worth (USD)') or 0)
            score = record.get('Credit Score')
            scale = record.get('Credit Score Scale')
            if scale in ('FICO', 'FICO-like', 'Canadian') and isinstance(score, int):
                # if net worth large but score low, nudge score up
                if net > 200000 and score < 500:
                    record['Credit Score'] = min(score + random.randint(100, 250), 900)
                # if net worth very small but score very high, nudge down slightly
                if net < 1000 and score > 800:
                    record['Credit Score'] = max(300, score - random.randint(50, 150))
            elif scale == 'Percent' and isinstance(score, int):
                if net > 200000 and score < 60:
                    record['Credit Score'] = min(100, score + random.randint(10,30))
        except Exception:
            logger.exception('Error in credit-networth rule')
...
class PIIGenerator:
    ...
    def export_csv(self, records_per_country=100):
        ...
        headers = [
        'Country', 'Full Name', 'Age', 'Birthdate', 'Gender', 'Marital Status', 'Race_Ethnicity','National ID',
        'Driving License', 'Passport', 'Tax ID', 'Bank Account', 'Credit Card',
        'Blood Group', 'Height (cm)', 'Eye Color', 'Skin Tone', 'Hair Color', 'Weight (kg)',
        'Medical Condition', 'Disability', 'Address', 'Work Address', 'Mobile', 'Landline', #'Postal Code'
        'Email', 'IP Address', 'MAC Address', 'Job Title', 'Workplace', 'Education Level',
                    ...
                    try:
                        ...
                        for i in range(records_per_country):
                            try:
                                ...
                                # enforce any registered rules to ensure referential integrity
                                try:
                                    if hasattr(factory, 'enforce_rules'):
                                        factory.enforce_rules(record)
                                except Exception as e:
                                    logger.error(f"Error enforcing rules for {country} record {i+1}: {e}")
